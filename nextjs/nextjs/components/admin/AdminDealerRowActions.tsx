...
import { BadgeDollarSign, Eye, KeyRound, MoreVertical, Pencil, ReceiptText, RefreshCcw, Trash2, Users, X } from "lucide-react";
import { resetStaffPasswordAction } from "@/actions/forms";
import { AdminDeleteDealerForm } from "@/components/admin/AdminDeleteDealerForm";
...
export function AdminDealerRowActions({
  username,
  displayName,
  canDelete,
  redirectPath,
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-3 border-b border-border/60 pb-3">
              <div className="min-w-0 space-y-2">
                <h3 className="text-lg font-semibold text-foreground">Dealer Details</h3>
            ...
            <div className="mb-3 grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                ...
                <p className="mt-0.5 text-sm font-semibold text-foreground">Dealer</p>
              </div>
              <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Login</p>
                <p className="mt-0.5 truncate text-sm font-medium text-foreground">{loginLabel}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Parent reseller</p>
                <p className="mt-1 truncate text-sm font-medium text-foreground">{resellerLogin || "—"}</p>
              </div>
              <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
                <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Manager</p>
                <p className="mt-1 truncate text-sm font-medium text-foreground">{managerLogin || "—"}</p>
              </div>
              <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2">
          ...
          >
            ...
            <form action={resetStaffPasswordAction} className="mt-4 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <PasswordInputWithToggle id={`dealer-reset-password-${username}`} name="password" required autoComplete="new-password" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Confirm password</label>
                <PasswordInputWithToggle
                  id={`dealer-reset-password-confirm-${username}`}
