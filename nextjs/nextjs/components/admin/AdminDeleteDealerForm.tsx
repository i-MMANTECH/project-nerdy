...
import { deleteAdminDealerAction } from "@/actions/forms";
import { Button } from "@/components/ui/button";
import { InlineConfirmAction } from "@/components/ui/InlineConfirmAction";
import { cn } from "@/lib/cn";

export function AdminDeleteDealerForm({
  username,
  canDelete,
  redirectPath,
  buttonLabel = "Del",
  ...
  buttonLabel?: string;
  className?: string;
  /** Row ⋮ menu: icon + label row (no solid red block). */
  menuItem?: boolean;
  onPanelOpenChange?: (open: boolean) => void;
  /** Render confirm open on mount; use with `positionAnchorRef` and a menu item that closes the portal first. */
  ...
  if (!canDelete) {
    return (
      <span
        ...
        title="You can't delete this dealer account"
      >
  ...
  return (
    <InlineConfirmAction
      action={deleteAdminDealerAction}
