...
import { ResellerDealerRowActions } from "@/components/portal/ResellerDealerRowActions";
import { listDealersOwnedByReseller, type ResellerPortalDealerRow } from "@/lib/data";
...
export async function ResellerDealersPage({
  ...
  const all = await listDealersOwnedByReseller(resellerUsername);
  const rows = qRaw
    ? all.filter((r: ResellerPortalDealerRow) => {
        const statusLabel = r.status === "A" ? "active" : "inactive";
        const hay = [r.username, r.name, String(r.userCount), String(r.credits), statusLabel].join(" ").toLowerCase();
  ...
  return (
    <div className="mx-auto w-full max-w-[min(100%,1920px)] space-y-5 pb-10">
      ...
      <PageHeader
        title="Dealers"
        breadcrumb="Dealer accounts under your reseller login."
        showBack={false}
        actions={
          <form action="/reseller/dealers" method="get" className="w-full min-w-0 sm:max-w-md">
            <label className="sr-only" htmlFor="srl-dealer-q">
              Search dealers
            </label>
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    <p className="text-sm font-medium text-foreground">
                      {qRaw ? "No dealers match your search" : "No dealers under this reseller yet"}
                    </p>
                    <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                      {qRaw ? "Clear the search or try another keyword." : "Use Add dealer to create the first account."}
                    </p>
                  </td>
                </tr>
              ) : null}
              {rows.map((r: ResellerPortalDealerRow) => (
                <tr
                  key={r.username}
                  className="border-b border-border/40 transition-colors last:border-0 hover:bg-primary/5 dark:hover:bg-muted/15"
                >
                  <td className={`${td} font-medium`}>{r.name || "—"}</td>
                  <td className={td}>
                    <Link
                      href={`/reseller/dealers/${encodeURIComponent(r.username)}`}
                  ...
                  <td className={cn(td, "text-right tabular-nums")}>
                    {hasPositiveCount(r.userCount) ? (
                      <Link
                        href={`/reseller/dealers/${encodeURIComponent(r.username)}/users`}
                  ...
                  <td className={`${td} text-center`}>
                    {r.status === "A" ? (
                      <span className="inline-flex rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-500/30 dark:text-emerald-300">
                        Active
                      </span>
                    ) : (
                      <span className="inline-flex rounded-full bg-rose-500/15 px-2.5 py-0.5 text-xs font-semibold text-rose-700 ring-1 ring-rose-500/30 dark:text-rose-300">
                        Inactive
                      </span>
                    )}
                  </td>
                  <td className={`${td} text-center`}>
                    <ResellerDealerRowActions username={r.username} displayName={r.name || r.username} canDelete={r.canDelete} />
                  </td>
                </tr>
              ))}
            </tbody>
