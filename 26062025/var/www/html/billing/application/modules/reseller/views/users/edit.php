                ...
                <div class="col-md-3">
                    <div class="panel panel-flat">
                        ...
                        <div class="panel-body">
                            <?php echo form_open('reseller/users/edit/'.$row->account,array('class'=>'','id'=>'form_sample_3'));?>
...
$( document ).ready(function() { 
    ...
    $('.btn-pre-transaction').click(function () {
        ...
        if (valChecked == 'RCDT') {
            ...
            let url = '/index.php/reseller/users/preCheckRenewal';
    ...
    $.get("<?= site_url('reseller/users/get_packages') ?>", function(data) {
    const res = JSON.parse(data);
    const selectId = <?= $stalker->tariff_plan_id; ?>; // Removed "echo"
    
    // Clear old options except [Default]
    $('#tariff_custom').find('option:not([value="0"])').remove();

    // Append new options
    $.each(res.data, function(index, package) {
        var selected = (package.id == selectId) ? 'selected="selected"' : '';
        $('#tariff_custom').append('<option value="' + package.id + '" ' + selected + '>' + package.name + '</option>');
    });
    
    console.log(res);
    });
    $.get("<?= site_url('reseller/users/get_custom_package/') ?>" + <?= $stalker->id; ?>, function(data) {
        const res = JSON.parse(data);    
        $('#show_packages .well').append(res.data);
        console.log(res);
    });
});
