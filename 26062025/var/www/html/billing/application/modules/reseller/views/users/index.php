  ...
  <div class="page-header">
    <div class="page-header-content">
      <div class="page-title">
         ...
         <a href="<?= site_url('reseller/users/add');?>" class="btn btn-primary btn-sm"> <i class="icon-add"></i> ADD NEW </a>
      </div>
        ...
        <div class="panel panel-flat">
          <div class="panel-heading">
            ...
            <div style="display: none;" class="actiontools">
                  ...
                  <a href="<?= site_url('reseller/users/index'); ?>" class="btn <?php if(empty($q)) {?>active <?php } ?> btn-default btn-sm text-primary"> ALL (<?= $total_users; ?>) </a>
                  <a href="<?= site_url('reseller/users/index?status=active'); ?>" class="btn btn-default text-success <?php if(!empty($q) && $q=='active') {?>active <?php } ?>  btn-sm"> ACTIVE (<?= $active_users; ?>) </a>
                  <a href="<?= site_url('reseller/users/index?status=expired'); ?>" class="btn btn-default text-danger <?php if(!empty($q) && $q=='expired') {?>active <?php } ?> btn-sm"> EXPIRED (<?= $expired_users; ?>) </a>
           ...
           <table class="table table-bordered table-striped" id="reseller_table" data-url="<?= site_url('reseller/users/data_list'); ?>">
            <thead>
              <tr>
  ...
  <script>
    ...
    function reset(element, account) {
      if (confirm(`Do you really want to reset this device MAC data?`)) {
          window.location.replace("<?= site_url('reseller/users/reset/') ?>"+ account);
      } 
    }
    function renewselected() {
        var table = $('#reseller_table').DataTable();
        ...
        $.ajax({
            url: '<?= site_url("reseller/users/renew_one_month_bulk"); ?>',
            method: 'POST',
            data: { checkedData: checkedData, validity: validity,[csrfName]: csrfHash}, // from your script block},
            success: function(response) {
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      window.location.href = '<?= site_url("reseller/users"); ?>';
                  }
              });
            },
            error: function(err) {
    ...
    $(document).ready(()=>{
      ...
      $('#reseller_table tbody').on('change', 'input.row-select', function () {
        updateSelectedCount();
      });
