...
class Tickets extends ManagerController {
	...
	public function index()	{
		...
		$did = $this->dealer_model->get_user_id($this->user['username'])->id;
		$this->data['dealer_id'] = $did;

		$this->data['tickets_enabled'] = $this->dealer_model->tickets_enable($this->user['username']);

		if($this->data['tickets_enabled']==1) {
			...
			$tickets_query = "SELECT id, subject, status_id, priority_id, category_id, updated_at, user_id, agent_id FROM ".$this->db->dbprefix('tickets')." WHERE user_id='".$did."' and status_id<>'2'";
		}
	...
	public function create() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			redirect(site_url('manager/tickets'), 'refresh');
	...
	public function comment() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($this->db->insert('tickets_comments', $comment_data)) {
				...
				redirect(site_url('manager/tickets/view/' . $redirect), 'refresh');
			}
		}
	}
	...
	public function dashboard() {
		...
		$this->load->view('manager/tickets_dashboard', $data);
	}
