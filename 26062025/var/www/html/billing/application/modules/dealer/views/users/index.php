          ...
          <div class="panel-heading">
            ...
            <div style="display: none;" class="actiontools">
                <?php $q = $this->input->get('status'); ?>
                  <a href="<?= site_url('dealer/users/index'); ?>" class="btn <?php if(empty($q)) {?>active <?php } ?> btn-default btn-sm text-primary total_users">  ALL (<?= $total_users; ?>)  </a>
                  <a href="<?= site_url('dealer/users/index?status=active'); ?>" class="btn btn-default text-success <?php if(!empty($q) && $q=='active') {?>active <?php } ?>  btn-sm"> ACTIVE (<?= $active_users; ?>) </a>
                  <a href="<?= site_url('dealer/users/index?status=expired'); ?>" class="btn btn-default text-danger <?php if(!empty($q) && $q=='expired') {?>active <?php } ?> btn-sm"> EXPIRED (<?= $expired_users; ?>) </a>
           ...
           <table class="table table-bordered table-striped "  id="users_table" data-url="<?= site_url('dealer/users/data_list'); ?>">
            <thead>
              <tr>
  ...
  <script>
    ...
    function reset(element, account) {
      if (confirm(`Do you really want to reset this device MAC data?`)) {
          window.location.replace("<?= site_url('dealer/users/reset/') ?>"+ account);
      } 
    }
    ...
    function renewselected() {
        ...
        $.ajax({
            url: '<?= site_url("dealer/users/renew_one_month_bulk"); ?>',
            method: 'POST',
            ...
            success: function(response) {
              ...
              Swal.fire({
                  ...
                  if (result.isConfirmed) {
                      window.location.href = '<?= site_url("dealer/users"); ?>';
                  }
              });
            },
            error: function(err) {
    ...
    $(document).ready(()=>{
      ...
      $('#users_table tbody').on('change', 'input.row-select', function () {
        updateSelectedCount();
      });
