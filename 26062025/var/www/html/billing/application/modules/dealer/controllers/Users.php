...
class Users extends DealerController {
    ...
    public function data_list(){
        ...
        $result = $this->users_model->get_user_by_status([$userid], $status, $length, $start, $orderBy, $orderDir, $searchValue);
        ...
        foreach ($users as $row) {
            ...
            $data[] = [
                ...
                '<td>' . $row['account'] . '</td>',
                '<td><a href="' . site_url( 'dealer/users/edit/' . $row['account']) . '">' . $row['mac'] . '</a></td>',
                ...
                '<td>' .
                    ...
                    form_open('dealer/users/renewOneMonth/' . $row["account"], ['class' => 'form-horizontal']) .
                    ...
                    form_close() .
                '</td>',
                '<td>' . $this->button($row["expires"], "dealer", $row["account"]) . '</td>'
            ];
    ...
    public function edit($username = NULL) {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($is_updated['status'] == TRUE) {
                ...
                $this->msg('User account was updated successfully!');
                redirect('dealer/users/edit/'.$username, 'refresh');
            }
        } else {
    ...
    private function check_validity_credits($validity, $callback_name) {
        ...
        $username = $this->user['username'];
        $remaining_credits = $this->transaction_model->get_credit_balance($this->userinfo['username']);
      
        if ($remaining_credits < intval($validity)) {
    ...
    private function check_validity_free_trial($validity, $callback_name) {
        ...
        // var_dump($mac_db_data->num_rows()); die();
            ...
            if ($type == "RCDT") {
                ...
                if ($credits === 'FREE_TRIAL') {
                    ...
                    if (!$isRetryTrial && $countUsingTrial > 0) {
                        log_debug_msg("dealer/controllers/users.php/renew(): You have already tried it. You cannot use it again");
                        $this->msg("You have already tried it. You cannot use it again.", 'danger');
                        redirect('dealer/users/edit/'.$username, 'refresh');
                    }

                    if ($isRetryTrial && $countUsingTrial >= $numberRetryTrial) {
                        log_debug_msg("dealer/controllers/users.php/renew(): $username has exceeded the free trial usage limit.");
        ...
        if ($sql->num_rows() == 0 || empty($login)) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($send_msg == true) {
                    ...
                    redirect('dealer/users/edit/'.$login, 'refresh');
                    // redirect('dealer/users', 'refresh');
                } else {
